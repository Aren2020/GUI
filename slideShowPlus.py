...
class SlideShowPlus(SlideShow):
    ...
    def makeWidgets(self):
        ...
        s = Scale(label = 'Speed: msec delay',command = self.onScale,from_=0,to = 3000, resolution = 50,
                                                                showvalue = YES,length = 400,
                                                                tickinterval = 250,orient = 'horizontal')
    ...
    def loadNote(self):
        if self.image and self.editorUp:
            ...
            try:
                ...
                self.editor.clearAllText() #примечание может отсутсвовать
    
    def onHelp(self):
        showinfo('About PyView',
                 'PyView version 1.2\nMay, 2010\n(1.1 July,1999)\nrewritten in (21.05.2023)\n'+
                 'An image slide show\nProgramming Python 4E')

if __name__=='__main__':
    import sys
    picdir = r'C:\Users\User\Pictures\Screenshots'
    if len(sys.argv) >=2:
        picdir = sys.argv[1]
